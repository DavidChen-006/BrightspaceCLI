...
| --- | --- | --- |
| A-01 | Node 20 floor suffices | RESOLVED (Brightspace-Bar): `engines.node >=20`, `.nvmrc` 20, CI matrix Node 20 + 22 (`session-capture/package.json`, `.github/workflows/ci.yml:26-27`). **Web wave overrides: Node 20 is EOL (2026-04-30) and commander 15 needs ≥22.12 → floor is Node 22.12.** `docs/evidence/node-toolchain-web.md` A-01. |
...
| A-03 | commander introspection suffices for `schema --json` | RESOLVED (web): commander 15 public typings expose `commands`, `options`, `registeredArguments`, `name()`, `aliases()`, `description()`, `Option.{long,short,envVar,defaultValue,argChoices,hidden}`; `.env()`; `exitOverride` + `configureOutput`; usage-error default exit 1 → map to 2. `docs/evidence/node-toolchain-web.md` A-03. |
| A-04 | plain `tsc` build is enough for the bin | RESOLVED (web): tsc preserves `#!` (TS #2749, fixed 1.6); npm `bin` needs the shebang; TS 7.0 `tsc` + `tsx` for dev; no bundler. `docs/evidence/node-toolchain-web.md` A-04. |
| A-05 | biome for lint/format | RESOLVED (web): Biome 2.5.x, `biome check --write` / `biome ci` (https://biomejs.dev/reference/cli/). `docs/evidence/node-toolchain-web.md` A-05. |
| A-06 | postinstall `playwright install chromium` acceptable | REVISED (web): no postinstall; depend on `playwright-core`; lazy `install chromium` from `auth doctor`/`auth login` (~300 MB on disk) or `BS_BROWSER_CHANNEL=chrome` to use installed Google Chrome. `docs/evidence/node-toolchain-web.md` A-06. |
...
| A-24 | JWT `exp` claim decodable, ~1 h | PARTIAL (Brightspace-Bar): no mint body ever recorded; 3600 s is a documented prior (`PollPolicyTests.swift:15`); no `exp` decoding exists. `bs` decodes `exp` defensively and falls back to 3600 s − 60 s. Web: `Buffer.from(p, "base64url")` (Node ≥15.7) → `JSON.parse().exp`. `docs/evidence/node-toolchain-web.md` A-24. |
...
| A-26 | state dir conventions per OS | PARTIAL (gogcli): macOS = `os.UserConfigDir()` → `~/Library/Application Support/<app>`; Linux/BSD split config `~/.config`, data `~/.local/share`, state `~/.local/state`; overrides must be absolute; dirs 0700, secret files 0600 atomic (`internal/config/layout.go:245-286, 432-439`). Sweep §11. Web: XDG v0.8 + Apple guide; use `env-paths` 4 (`data` dir as root). `docs/evidence/node-toolchain-web.md` A-26. |
